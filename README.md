...
git init
git add .
git commit -m "Табло для волейболу"
git branch -M main
git remote add origin https://github.com/<користувач>/volleyball_scoreboard.git
git push -u origin main
```

Далі в репозиторії: **Settings → Pages → Source → GitHub Actions**. Робочий процес
`.github/workflows/deploy.yml` уже в проєкті: він проганяє тести, і лише якщо вони
зелені, публікує табло. Тести й CI на хостинг не їдуть.

Через хвилину табло буде на `https://<користувач>.github.io/volleyball_scoreboard/`.
Це https, тож працює все: встановлення на домашній екран, офлайн-кеш, Wake Lock.

Шляхи в проєкті відносні (`./match.js`, `./sw.js`), тому підтека репозиторію в адресі
нічого не ламає — окремий домен не потрібен.

**Після кожної зміни файлів піднімайте версію кешу в `sw.js`**
(`volley-score-v2` → `v3`). Інакше у вже встановлених застосунках лишиться стара
версія: service worker віддає її з кешу й на сервер не піде.

Будь-яка інша статика теж підходить: Netlify, Vercel, nginx — завантажити папку як є
